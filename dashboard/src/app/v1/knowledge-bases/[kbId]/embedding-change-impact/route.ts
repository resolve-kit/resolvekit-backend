...
import { KBServiceError, kbEmbeddingChangeImpact } from "@/lib/server/kb-service";

export const dynamic = "force-dynamic";

type EmbeddingChangePayload = {
...
export async function POST(
  ...
  const body = await readJson<EmbeddingChangePayload>(request);
  if (!body || typeof body.embedding_profile_id !== "string") {
    return detail(422, "embedding_profile_id is required");
  }

  const { kbId } = await context.params;

  try {
    const payload = await kbEmbeddingChangeImpact(actorContext(developer), kbId, body.embedding_profile_id);
