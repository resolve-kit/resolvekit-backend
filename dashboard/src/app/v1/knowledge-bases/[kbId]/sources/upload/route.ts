...
import { KBServiceError, kbSourcesAddUpload } from "@/lib/server/kb-service";
...
export async function POST(
  ...
  const body = await readJson<UploadSourcePayload>(request);
  if (!body || typeof body.title !== "string" || typeof body.content !== "string") {
    return detail(422, "Invalid upload source payload");
  }

  const { kbId } = await context.params;

  try {
    const payload = await kbSourcesAddUpload(actorContext(developer), kbId, {
      title: body.title,
