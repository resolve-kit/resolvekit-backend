...
describe("dashboard security config", () => {
  ...
  it("allows development startup with a derived local Fernet key", () => {
    ...
    vi.stubEnv("IAA_JWT_SECRET", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef");
    vi.stubEnv("IAA_ENCRYPTION_KEY", "change-me-generate-with-python-fernet");
  ...
  it("still requires an explicit Fernet key in production", () => {
    ...
    expect(() => assertDashboardSecurityConfig()).toThrowError("IAA_ENCRYPTION_KEY must be a valid Fernet key");
  });
